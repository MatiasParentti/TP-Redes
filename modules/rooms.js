...
//salir de la sala , alimina si es 0
    ...
    if (users.delete(user)) {
      ...
      if (users.size === 0) {
        ...
        logEvent(
          ...
          null,
          null,
          `Sala ${roomName} eliminada por estar vacía`
        );
      }
    }
  }
...
function notifyRoomUsers(room, message, excludeWs = null) {
  ...
  logEvent(
    ...
    null,
    null,
    `Notificando ${clientsInRoom.length} usuarios en ${room}: ${message}`
  );
